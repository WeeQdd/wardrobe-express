...
def get_user_catalog_btns(level: int, categories: list) -> InlineKeyboardMarkup:
    ...
    btns["⬅️ Назад"] = MenuCallBack(level=0, menu_name="main").pack()
    ...
    *,
    ...
    btns = {
        ...
        **{
        ...
        "⬅️ Назад": MenuCallBack(level=1, menu_name="in_stock").pack(),
    }
...
def get_batch_item_list_btns(orders: list) -> InlineKeyboardMarkup:
    keyboard = InlineKeyboardBuilder()

    for order in orders:
        keyboard.button(
            text=f"✏️ #{order.id} {order.product_name[:22]}",
            callback_data=f"batchitem_{order.id}",
        )

    keyboard.button(text="✅ Отправить заявку", callback_data="batch_finalize")
    keyboard.button(text="🛍 В наличии", callback_data=MenuCallBack(level=1, menu_name="in_stock").pack())
    keyboard.button(text="📝 Заказ", callback_data=MenuCallBack(level=1, menu_name="order").pack())
    keyboard.button(text="🗑 Сбросить заявку", callback_data="batch_cancel")
    keyboard.button(text="🏠 Меню", callback_data=MenuCallBack(level=0, menu_name="main").pack())
    keyboard.adjust(1, 1, 2, 1, 1)
    return keyboard.as_markup()


def get_batch_item_detail_btns(order_id: int) -> InlineKeyboardMarkup:
    return get_inline_mix_btns(
        btns={
            "✏️ Название": f"batchedit_name_{order_id}",
            "📏 Размер": f"batchedit_size_{order_id}",
            "🔗 Ссылка": f"batchedit_url_{order_id}",
            "💰 Цена": f"batchedit_price_{order_id}",
            "🗑 Удалить позицию": f"batchdelete_{order_id}",
            "⬅️ К заявке": "batch_open",
        },
        sizes=(2, 2, 1, 1),
    )
...
def get_saved_profile_btns() -> InlineKeyboardMarkup:
    return get_inline_mix_btns(
        btns={
            ...
            "✏️ Указать новые данные": "order_use_new",
        },
        sizes=(1, 1),
    )
...
def get_client_list_btns(clients: list) -> InlineKeyboardMarkup:
    ...
    for client in clients:
        ...
        btns[f"👤 {name[:38]}"] = f"client_{client.id}"
...
def get_user_order_detail_btns(order_id: int, *, can_cancel: bool) -> InlineKeyboardMarkup:
    ...
    btns["⬅️ Назад к заявкам"] = MenuCallBack(level=1, menu_name="myorders").pack()
    btns["🏠 Меню"] = MenuCallBack(level=0, menu_name="main").pack()
...
def get_admin_order_detail_btns(
    ...
    *,
    ...
    keyboard.button(text="⬅️ Назад к списку", callback_data="adminorders_list")
